...
def configurar_servidor(configuracoes):
    ...
    def buscar():
        ...
        # Verifica se o objeto está localmente (no banco de dados vinculado)
        objeto_local = collection.find_one({'_id': id_objeto})
        if objeto_local:
            objeto_local['servidor'] = id_servidor  # Adiciona a informação do servidor

            # Mensagem personalizada para informar em qual servidor/porta o objeto foi encontrado, a informação vai aparecer no terminal.
        ...
        # Se nenhum vizinho possui o objeto, retorna uma resposta vazia
        return json.dumps({})
...
# Configuração e leitura do JSON do servidor principal
